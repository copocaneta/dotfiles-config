...
local function v_notify(cmd)
  return string.format("<cmd>call VSCodeCallRange('%s', 1)<CR>", cmd)
end
...
local function set_keymaps()
  ...
  vim.api.nvim_set_keymap("n", "<C-w>=", '<Cmd>lua vim.fn.VSCodeNotify("workbench.action.evenEditorWidths")<CR>', opts)
  vim.api.nvim_set_keymap("x", "<C-w>=", '<Cmd>lua vim.fn.VSCodeNotify("workbench.action.evenEditorWidths")<CR>', opts)
  vim.api.nvim_set_keymap(
    "n",
    "<C-w>>",
  ...
  -- vim.api.nvim_set_keymap( -- change o in normal mode to also auto indent using VSCode
  --   "n",
  --   "o",
  ...
  keymap("n", "<Space>fs", notify("workbench.action.findInFiles"), { silent = true }) -- find files
...
local autocmd_group = augroup("MyAutocommands", { clear = true })

-- Add the BufEnter autocommand
